...
;;
;; `ctx` is an environment context extended with the atom table: `:index` maps
;; atom expressions to indices (in order of first appearance) and `:atoms` maps
;; back.

(defn- atom-table
  "A fresh atom table on top of the environment context `env-ctx`. The result is
  itself a context, so the normalizer's lemma lookups read `:env` from it."
  [env-ctx]
  (assoc env-ctx :index {} :atoms []))
...
(defn- mmul
  ...
  [ctx m1 m2]
  (cond
    (empty? m1) [m2 (k/lemma-in ctx "Int.one_mul" (mon-expr ctx m2))]
    (empty? m2) [m1 (k/lemma-in ctx "Int.mul_one" (mon-expr ctx m1))]
    :else
    (let [v (first m1) w (first m2)]
      (if (<= v w)
        (let [atom (get-in ctx [:atoms v])
          ...
          [(into [v] m)
           (k/trans (k/lemma-in ctx "Int.mul_assoc" atom (mon-expr ctx (subvec m1 1)) (mon-expr ctx m2))
                    (k/congr-mul (k/refl atom) p))])
        (let [atom (get-in ctx [:atoms w])
              [m p] (mmul ctx m1 (subvec m2 1))]
          [(into [w] m)
           (k/trans (k/lemma-in ctx "Int.mul_left_comm" (mon-expr ctx m1) atom (mon-expr ctx (subvec m2 1)))
                    (k/congr-mul (k/refl atom) p))])))))
...
(defn- padd
  ...
  [ctx P Q]
  (cond
    (empty? P) [Q (k/lemma-in ctx "Int.zero_add" (poly-expr ctx Q))]
    (empty? Q) [P (k/lemma-in ctx "Int.add_zero" (poly-expr ctx P))]
    :else
    (let [[c1 m1 :as t1] (first P), P' (subvec P 1)
      ...
      (cond
        ...
        (let [[R p] (padd ctx P' Q)]
          [(into [t1] R)
           (k/trans (k/lemma-in ctx "Int.add_assoc" te1 (poly-expr ctx P') (poly-expr ctx Q))
                    (k/congr-add (k/refl te1) p))])

        (pos? order)
        (let [[R p] (padd ctx P Q')]
          [(into [t2] R)
           (k/trans (k/lemma-in ctx "Int.add_left_comm" (poly-expr ctx P) te2 (poly-expr ctx Q'))
        ...
        (let [m     (mon-expr ctx m1)
              ...
              ;; (t1 + P') + (t2 + Q') = (t1 + t2) + (P' + Q')
              regroup (k/trans
                       (k/lemma-in ctx "Int.add_assoc" te1 (poly-expr ctx P') (poly-expr ctx Q))
                       (k/congr-add (k/refl te1)
                                    (k/lemma-in ctx "Int.add_left_comm" (poly-expr ctx P') te2 (poly-expr ctx Q')))
                       (k/symm (k/lemma-in ctx "Int.add_assoc" te1 te2
                                        (k/add (poly-expr ctx P') (poly-expr ctx Q')))))
              ;; c1 * m + c2 * m = c3 * m
              combine (k/trans (k/symm (k/lemma-in ctx "Int.add_mul" (k/lit c1) (k/lit c2) m))
                               (k/congr-mul (ground :add c1 c2) (k/refl m)))
              summed  (k/trans regroup (k/congr-add combine p))]
          (if (zero? c3)
            [R (k/trans summed
                        (k/congr-add (k/lemma-in ctx "Int.zero_mul" m) (k/refl (poly-expr ctx R)))
                        (k/lemma-in ctx "Int.zero_add" (poly-expr ctx R)))]
            [(into [[c3 m1]] R) summed]))))))
...
(defn- ttmul
  ...
  (let [M1 (mon-expr ctx m1)
    ...
    [[[c3 m]]
     (k/trans (k/lemma-in ctx "Int.mul_assoc" (k/lit c1) M1 (k/mul (k/lit c2) M2))
              (k/congr-mul (k/refl (k/lit c1))
                           (k/lemma-in ctx "Int.mul_left_comm" M1 (k/lit c2) M2))
              (k/symm (k/lemma-in ctx "Int.mul_assoc" (k/lit c1) (k/lit c2) (k/mul M1 M2)))
              (with-rhs (k/congr-mul (ground :mul c1 c2) p-mon) t3)
              (k/symm (k/lemma-in ctx "Int.add_zero" t3)))]))
...
(defn- tmul
  ...
  (let [te (term-expr ctx t)]
    (if (empty? Q)
      [[] (k/lemma-in ctx "Int.mul_zero" te)]
      (let [u (first Q), Q' (subvec Q 1)
            [T p1] (ttmul ctx t u)
            [R2 p2] (tmul ctx t Q')
            [R p3] (padd ctx T R2)]
        [R (k/trans (k/lemma-in ctx "Int.mul_add" te (term-expr ctx u) (poly-expr ctx Q'))
                    (k/congr-add p1 p2)
                    p3)]))))
...
(defn- pmul
  ...
  [ctx P Q]
  (if (empty? P)
    [[] (k/lemma-in ctx "Int.zero_mul" (poly-expr ctx Q))]
    (let [t (first P), P' (subvec P 1)
          [R1 p1] (tmul ctx t Q)
          [R2 p2] (pmul ctx P' Q)
          [R p3] (padd ctx R1 R2)]
      [R (k/trans (k/lemma-in ctx "Int.add_mul" (term-expr ctx t) (poly-expr ctx P') (poly-expr ctx Q))
                  (k/congr-add p1 p2)
                  p3)])))
  ...
  (let [[op a b] ir]
    (case op
      :lit (if (zero? a)
             ...
             (let [c (k/lit a)]
               [[[a []]]
                (with-rhs (k/symm (k/trans (k/lemma-in ctx "Int.add_zero" (k/mul c k/one))
                                           (k/lemma-in ctx "Int.mul_one" c)))
                  (poly-expr ctx [[a []]]))]))
      :atom (let [x (get-in ctx [:atoms a])]
              [[[1N [a]]]
               (with-rhs (k/symm (k/trans (k/lemma-in ctx "Int.add_zero" (k/mul k/one (k/mul x k/one)))
                                          (k/lemma-in ctx "Int.one_mul" (k/mul x k/one))
                                          (k/lemma-in ctx "Int.mul_one" x)))
                 (poly-expr ctx [[1N [a]]]))])
      ...
      :neg (let [[A pa] (normalize ctx a)
                 ...
                 [R p] (pmul ctx N A)]
             [R (k/trans (k/lemma-in ctx "Int.neg_eq_neg_one_mul" (:lhs pa))
                         (k/congr-mul pn pa)
                         p)])
      :sub (let [[R p] (normalize ctx [:add a [:neg b]])
                 [_ pa] (normalize ctx a)
                 [_ pb] (normalize ctx b)]
             [R (k/trans (k/lemma-in ctx "Int.sub_eq_add_neg" (:lhs pa) (:lhs pb)) p)]))))
...
(defn- const-rule [ctx s]
  (let [ci (or (k/lookup ctx s)
...
(defn- resolve-rule [ctx lctx sym]
  (let [s (str sym)]
...
(defn normalize-terms
  ...
  `[[nf proof] …]` with `proof : term = nf` (a proof map). The one-argument
  form uses the global environment (deprecated)."
  ([terms] (normalize-terms (k/base-ctx) terms))
  ([env-ctx terms]
...
(defn prove-eq
  ...
  forms if the sides differ as polynomials. Without a context it uses the
  global environment (deprecated)."
  ([lhs rhs] (prove-eq (k/base-ctx) lhs rhs))
  ([env-ctx lhs rhs]
   (let [[ctx l-ir] (reify-term (atom-table env-ctx) lhs)
...
(defn linear-combination
  ...
  Without a context it uses the global environment (deprecated)."
  ([lhs rhs hyps] (linear-combination (k/base-ctx) lhs rhs hyps))
  ([ctx lhs rhs hyps]
   (let [diff (fn [{:keys [lhs rhs]}] (k/sub lhs rhs))
         ...
         to-zero (reduce (fn [acc [c {:keys [lhs rhs term]}]]
                           (k/congr-add acc
                                        (k/congr-mul (k/refl c)
                                                     (k/lemma-in ctx "Int.sub_eq_zero_of_eq"
                                                                 lhs rhs term))))
                         (k/refl rhs)
                         hyps)]
     (k/trans (prove-eq ctx lhs spread)
              (assoc to-zero :lhs spread :rhs zeros)
              (prove-eq ctx zeros rhs)))))
...
(defn int-ring
  ...
  namespace docstring)."
  [{:keys [env] :as ps} rule-syms]
  (let [ctx {:env env}
        goal (or (proof/current-goal ps)
                 (throw (ex-info "int_ring: no goals" {})))
        [lhs rhs] (goal-sides (:type goal))
        rules (mapv #(resolve-rule ctx (:lctx goal) %) rule-syms)
        ...
        p (k/trans pl (prove-eq ctx (:rhs pl) (:rhs pr)) (k/symm pr))]
    (-> ps
...
(defn int-ring-split
  ...
  (let [ctx {:env env}
        ...
        unfold (into #{} (keep :unfold) (map #(resolve-rule ctx (:lctx goal) %) rule-syms))
        st (tc/attach-lctx (tc/mk-tc-state env) (:lctx goal))]
...
(defn register!
  "IO edge. Registers `int_ring` and `int_ring_split` with Ansatz's global
  tactic registry. The registry is process state, not part of the kernel
  environment, so it is set up here rather than by [[install]].
  `emmy.ansatz.install` runs it before installing. Idempotent."
  []
...
(defn install
  "Pure. This namespace declares nothing in the environment: its work is the
  proof-producing normalizer and the tactics of [[register!]]."
  [ctx]
  ctx)
